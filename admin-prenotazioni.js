// admin-prenotazioni.js v3.2.3 - Ripristino completo + fix unico tasto conferma
(function(){
  ...
  function showSkeletonLoading() {
    ...
    if (!container) return;
    const skeletons = Array(6).fill(0).map(() => '<div class="col-md-4"><div class="skeleton-card"></div></div>').join('');
  ...
  function renderQuickStats() {
    ...
    statsDiv.innerHTML = STATI_TABELLA.map(stato => {
      const active = (currentFilters.stato === stato || (stato === 'Tutte' && (currentFilters.stato === 'tutti' || !currentFilters.stato))) ? 'btn-quickfilter-active' : '';
      const color = STATI_COLORI[stato].bg;
      const icon = STATI_COLORI[stato].icon;
      return `<button class="btn-quickfilter ${active} bg-${color}" onclick="window.quickStatFilter('${stato}')"><i class="fas fa-${icon} me-1"></i> ${stato}: <b>${statiCount[stato]}</b></button>`;
    }).join('');
  }
    ...
    if (filteredPrenotazioni.length === 0) {
      container.innerHTML = `
        <div class="empty-state">
          ...
          <div class="empty-state-text">${currentFilters.stato !== 'tutti' || currentFilters.ricerca ? 'Prova a modificare i filtri di ricerca' : 'Non ci sono prenotazioni al momento'}</div>
          ${currentFilters.stato !== 'tutti' || currentFilters.ricerca ? '<button class="btn btn-primary" onclick="window.quickStatFilter(\'Tutte\')">Mostra Tutte</button>' : '<button class="btn btn-success" onclick="showNewBookingModal()"><i class="fas fa-plus me-2"></i>Crea Prenotazione</button>'}
        </div>
      `;
      return;
    }
          ...
          <div class="col-md-4">
            <div class="card glass-card mb-3 shadow-sm border-0">
              <div class="card-body">
                ...
                ${p.stato === 'In attesa' ? `<button class="btn btn-success w-100 mt-3" onclick="window.confermaPrenotazione('${p.idPrenotazione || p.id}')"><i class="fas fa-check-circle me-1"></i>Conferma</button>` : ''}
              </div>
            </div>
          </div>
        `;
  ...
  function renderPrenotazioniTable() {
    ...
    const sorted = [...filteredPrenotazioni].sort((a, b) => new Date(b.giornoInizio) - new Date(a.giornoInizio));
    container.innerHTML = `<div class="card"><div class="card-header">Lista Prenotazioni</div><div class="card-body p-0"><div class="table-responsive"><table class="table table-hover mb-0"><thead><tr><th>ID</th><th>Cliente</th><th>Veicolo</th><th>Date</th><th>Orari</th><th>Destinazione</th><th>Stato</th><th class="text-end">Azioni</th></tr></thead><tbody>` + sorted.map(p => {
      const statoConfig = STATI_COLORI[p.stato] || STATI_COLORI['In attesa'];
      const dataInizio = p.giornoInizio ? new Date(p.giornoInizio).toLocaleDateString('it-IT') : '-';
      const dataFine = p.giornoFine ? new Date(p.giornoFine).toLocaleDateString('it-IT') : '-';
      const oraInizio = p.oraInizio || '';
      const oraFine = p.oraFine || '';
      return `<tr><td><strong>${p.idPrenotazione || p.id}</strong></td><td><span class="card-prenotazione-nome">${p.nomeAutista1 || '-'}</span><br><small class="text-muted"><i class="fas fa-phone me-1"></i>${p.cellulare || '-'}</small></td><td><span class="badge bg-secondary">${p.targa || '-'}</span></td><td>${dataInizio} → ${dataFine}</td><td>${oraInizio && oraFine ? oraInizio + ' → ' + oraFine : '-'}</td><td>${p.destinazione || '-'}</td><td><span class="badge bg-${statoConfig.bg}"><i class="fas fa-${statoConfig.icon} me-1"></i>${p.stato}</span></td><td class="text-end"><div class="btn-group btn-group-sm"><button class="btn btn-outline-primary" title="Dettagli" onclick="window.mostraDettaglioPrenotazione('${p.idPrenotazione || p.id}')"><i class="fas fa-eye"></i></button><button class="btn btn-outline-warning" title="Modifica" onclick="window.modificaPrenotazione('${p.idPrenotazione || p.id}')"><i class="fas fa-edit"></i></button><button class="btn btn-outline-danger" title="Elimina" onclick="window.eliminaPrenotazione('${p.idPrenotazione || p.id}')"><i class="fas fa-trash"></i></button>${p.pdfUrl ? `<a href="${p.pdfUrl}" target="_blank" class="btn btn-outline-secondary" title="PDF"><i class="fas fa-file-pdf"></i></a>` : ''}</div></td></tr>`;
  ...
  window.applicaFiltriPrenotazioni = function() {
    filteredPrenotazioni = allPrenotazioni.filter(p => {
      ...
      if (currentFilters.ricerca) {
        const searchText = [p.idPrenotazione, p.nomeAutista1, p.codiceFiscaleAutista1, p.targa, p.destinazione, p.cellulare, p.email].filter(Boolean).join(' ').toLowerCase();
  ...
  console.log('[ADMIN-PRENOTAZIONI] v3.2.3 loaded - Versione stabile completa + fix tasto conferma!');
})();
