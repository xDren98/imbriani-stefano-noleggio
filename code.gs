...
// ============ SOSTITUZIONE MAILAPP: inviaEmailConfermaCliente ============
function inviaEmailConfermaCliente(prenotazione){
  try {
    ...
    var corpo = '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>body{font-family:Arial,sans-serif;line-height:1.6;color:#333;max-width:600px;margin:0 auto}.header{background:linear-gradient(135deg,#0066FF,#004ECC);color:#fff;padding:20px;text-align:center}.content{padding:20px;background:#f9f9f9}.info-box{background:#fff;border-left:4px solid #0066FF;padding:15px;margin:15px 0}.status{background:#E3F2FD;padding:10px;border-radius:5px;text-align:center;font-weight:bold}.footer{background:#333;color:#fff;padding:15px;text-align:center;font-size:12px}.highlight{color:#0066FF;font-weight:bold}</style></head><body>' + '<div class="header"><h1>🚐 Imbriani Stefano Noleggio</h1><p>Conferma Prenotazione Ricevuta</p></div>' + '<div class="content"><h2>Gentile '+nomeCliente+',</h2><p>La Sua richiesta è stata <strong>ricevuta correttamente</strong> e sarà esaminata dal nostro staff.</p>' + '<div class="status">🔄 STATO: IN ATTESA DI CONFERMA</div>' + '<div class="info-box"><h3>📋 Dettagli Prenotazione</h3>' + '<p><strong>ID Prenotazione:</strong> <span class="highlight">'+(prenotazione.idPrenotazione || 'N/A')+'</span></p>' + '<p><strong>Veicolo:</strong> '+(prenotazione.targa || 'N/A')+'</p>' + '<p><strong>Dal:</strong> '+dataInizio+' alle '+(prenotazione.oraInizio || 'N/A')+'</p>' + '<p><strong>Al:</strong> '+dataFine+' alle '+(prenotazione.oraFine || 'N/A')+'</p>' + '<p><strong>Destinazione:</strong> '+(prenotazione.destinazione || 'Non specificata')+'</p></div>' + '<div class="info-box"><h3>⏰ Prossimi Passaggi</h3>' + '<p>• Il nostro team verificherà disponibilità e dettagli.</p>' + '<p>• Riceverà una comunicazione di <strong>conferma</strong> quando l’ordine sarà approvato.</p>' + '<p>• Le invieremo un <strong>promemoria</strong> 3 giorni prima della partenza.</p></div>' + '<div class="info-box"><h3>📞 Contatti</h3><p><strong>Email:</strong> '+CONFIG.EMAIL.FROM_EMAIL+'</p></div></div>' + '<div class="footer"><p>© 2025 Imbriani Stefano Noleggio</p></div></body></html>';
...
// ============ SOSTITUZIONE MAILAPP: inviaEmailConfermaPreventivo ============
function inviaEmailConfermaPreventivo(prenotazione){
  try {
    ...
    var dataFine = prenotazione.giornoFine ? new Date(prenotazione.giornoFine).toLocaleDateString('it-IT') : 'N/A';
    var nomeCliente = (prenotazione.autista1 && prenotazione.autista1.nomeCompleto) || 'Cliente';
    var corpo = '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>body{font-family:Arial,sans-serif;line-height:1.6;color:#333;max-width:600px;margin:0 auto}.header{background:linear-gradient(135deg,#22C55E,#16A34A);color:#fff;padding:20px;text-align:center}.content{padding:20px;background:#f9f9f9}.info-box{background:#fff;border-left:4px solid #22C55E;padding:15px;margin:15px 0}.status{background:#DCFCE7;padding:10px;border-radius:5px;text-align:center;font-weight:bold;color:#166534}.footer{background:#333;color:#fff;padding:15px;text-align:center;font-size:12px}.highlight{color:#22C55E;font-weight:bold}</style></head><body>' + '<div class="header"><h1>🚐 Imbriani Stefano Noleggio</h1><p>Prenotazione Confermata</p></div>' + '<div class="content"><h2>Gentile '+nomeCliente+',</h2><p>La informiamo che la Sua prenotazione è stata <strong>confermata</strong>.</p>' + '<div class="status">✅ PRENOTAZIONE CONFERMATA</div>' + '<div class="info-box"><h3>📋 Riepilogo</h3>' + '<p><strong>ID:</strong> <span class="highlight">'+(prenotazione.idPrenotazione || 'N/A')+'</span></p>' + '<p><strong>Veicolo:</strong> '+(prenotazione.targa || 'N/A')+'</p>' + '<p><strong>Dal:</strong> '+dataInizio+' alle '+(prenotazione.oraInizio || 'N/A')+'</p>' + '<p><strong>Al:</strong> '+dataFine+' alle '+(prenotazione.oraFine || 'N/A')+'</p>' + '<p><strong>Destinazione:</strong> '+(prenotazione.destinazione || 'Non specificata')+'</p></div>' + '<div class="info-box"><h3>📝 Note</h3><p>Per modifiche o informazioni aggiuntive, risponda a questa email o ci contatti.</p></div></div>' + '<div class="footer"><p>© 2025 Imbriani Stefano Noleggio</p></div></body></html>';
    inviaEmailViaGmailAPI(prenotazione.email, oggetto, corpo);
  } catch (error) { Logger.log('Errore invio email conferma (cliente): ' + error.message); }
}

// ============ SOSTITUZIONE MAILAPP: inviaEmailReminder ============
function inviaEmailReminder(prenotazione){
  try {
    ...
    var dataFine = prenotazione.giornoFine ? new Date(prenotazione.giornoFine).toLocaleDateString('it-IT') : 'N/A';
    var nomeCliente = (prenotazione.autista1 && prenotazione.autista1.nomeCompleto) || 'Cliente';
    var corpo = '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>body{font-family:Arial,sans-serif;line-height:1.6;color:#333;max-width:600px;margin:0 auto}.header{background:linear-gradient(135deg,#F59E0B,#D97706);color:#fff;padding:20px;text-align:center}.content{padding:20px;background:#f9f9f9}.info-box{background:#fff;border-left:4px solid #F59E0B;padding:15px;margin:15px 0}.footer{background:#333;color:#fff;padding:15px;text-align:center;font-size:12px}.highlight{color:#F59E0B;font-weight:bold}</style></head><body>' + '<div class="header"><h1>🚐 Imbriani Stefano Noleggio</h1><p>Promemoria Partenza</p></div>' + '<div class="content"><h2>Gentile '+nomeCliente+',</h2><p>Le ricordiamo che tra <strong>3 giorni</strong> inizierà il servizio prenotato.</p>' + '<div class="info-box"><h3>📋 Riepilogo</h3>' + '<p><strong>ID:</strong> <span class="highlight">'+(prenotazione.idPrenotazione || 'N/A')+'</span></p>' + '<p><strong>Veicolo:</strong> '+(prenotazione.targa || 'N/A')+'</p>' + '<p><strong>Dal:</strong> '+dataInizio+' alle '+(prenotazione.oraInizio || 'N/A')+'</p>' + '<p><strong>Al:</strong> '+dataFine+' alle '+(prenotazione.oraFine || 'N/A')+'</p>' + '<p><strong>Destinazione:</strong> '+(prenotazione.destinazione || 'Non specificata')+'</p></div>' + '<div class="footer"><p>© 2025 Imbriani Stefano Noleggio</p></div></body></html>';
    inviaEmailViaGmailAPI(prenotazione.email, oggetto, corpo);
