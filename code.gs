/**
 * IMBRIANI STEFANO NOLEGGIO - BACKEND v8.3.4
 * Backend Apps Script completo con:
 * - checkDisponibilita che ritorna {disponibile, conflitti}
 * - getVeicoli, getPrenotazioni
 * - updateStatiLive per aggiornare stati su PRENOTAZIONI e MANUTENZIONI
 */
...
function doGet(e){
  ...
  try{
    ...
    switch(action){
      ...
      case 'getSheet': return getSheetGeneric(p); // fallback generico
      default: return createJsonResponse({success:false,message:'Azione non supportata: '+action, available_actions:['health','getVeicoli','getPrenotazioni','checkDisponibilita','updateStatiLive','getSheet']},400);
    }
  }catch(err){ return createJsonResponse({success:false,message:'Errore server: '+err.message,error:String(err)},500); }
}
...
function updateStatiLive(){
  try{
    ...
    for (let i=1;i<valsP.length;i++){
      const r=valsP[i]; const stato=String(r[CONFIG.PRENOTAZIONI_COLS.STATO_PRENOTAZIONE-1]||'');
      if (stato==='Da confermare') continue; // non toccare
      const di = new Date(r[CONFIG.PRENOTAZIONI_COLS.GIORNO_INIZIO-1]); const df = new Date(r[CONFIG.PRENOTAZIONI_COLS.GIORNO_FINE-1]);
      let next = stato;
      if (stato==='Confermata' && today < di) next='Programmata';
      if ((stato==='Programmata' || stato==='Confermata') && today>=di && today<=df) next='In corso';
      if (stato==='In corso' && today>df) next='Completata';
    ...
    for (let i=1;i<valsM.length;i++){
      ...
      let next = stato;
      if (stato==='Programmata' && today>=di && today<=df) next='In corso';
      if (stato==='In corso' && today>df) next='Completata';
      if (next!==stato){ shM.getRange(i+1, CONFIG.MANUTENZIONI_COLS.STATO, 1, 1).setValue(next); }
    }
