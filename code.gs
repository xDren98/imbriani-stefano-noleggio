/**
 * IMBRIANI STEFANO NOLEGGIO - BACKEND v8.3.10 (completo)
...
function doGet(e){
  const p = e && e.parameter ? e.parameter : {}; const action = p.action || 'health'; const token = p.token || (p.Authorization ? p.Authorization.replace('Bearer ','') : '');
  try{
...
function doPost(e){
  try{
    ...
    let post = {}; try{ post = JSON.parse(e && e.postData ? (e.postData.contents || '{}') : '{}'); }catch(_){ return createJsonResponse({success:false,message:'Invalid JSON in request body'},400); }
    ...
    switch(action){ case 'getPrenotazioni':return getPrenotazioni(); case 'getVeicoli':return getVeicoli(); case 'creaPrenotazione':return creaPrenotazione(post); case 'aggiornaStato':return (typeof aggiornaStatoPrenotazione==='function')?aggiornaStatoPrenotazione(post):createJsonResponse({success:false,message:'aggiornaStato non implementata'},400); case 'setManutenzione':return (typeof setManutenzione==='function')?setManutenzione(post):createJsonResponse({success:false,message:'setManutenzione non implementata'},400); case 'aggiornaCliente':return aggiornaCliente(post); default:return createJsonResponse({success:false,message:'Azione POST non supportata: '+action},400); }
...
function createJsonResponse(data,status){ const s=status||200; const resp={...data,timestamp:new Date().toISOString(),version:CONFIG.VERSION,status:s}; return ContentService.createTextOutput(JSON.stringify(resp)).setMimeType(ContentService.MimeType.JSON); }
